<?php

// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Strings for component 'quiz_grading', language 'es_mx', branch 'MOODLE_35_STABLE'
 *
 * @package   quiz_grading
 * @copyright 1999 onwards Martin Dougiamas  {@link http://moodle.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['alldoneredirecting'] = 'Todos los intentos han sido calificados. Volviendo a la lista de preguntas';
$string['alreadygraded'] = 'Ya fue calificado';
$string['alsoshowautomaticallygraded'] = 'Mostrar también preguntas que han sido calificadas automáticamente';
$string['attemptstograde'] = 'Intentos para calificar';
$string['automaticallygraded'] = 'Calificados automáticamente';
$string['backtothelistofquestions'] = 'Volver a la lista de preguntas';
$string['bydate'] = 'Por fecha';
$string['bystudentfirstname'] = 'Por nombre de estudiante';
$string['bystudentidnumber'] = 'Por número ID del estudiante';
$string['bystudentlastname'] = 'Por apellido de estudiante';
$string['cannotgradethisattempt'] = 'No se puede calificar este intento';
$string['cannotloadquestioninfo'] = 'No se ha podido cargar la información sobre el tipo de pregunta específica';
$string['changeoptions'] = 'Cambiar opciones';
$string['essayonly'] = 'Las siguientes preguntas necesitan calificarse manualmente';
$string['grade'] = 'Calificación';
$string['gradeall'] = 'Calificar todos';
$string['gradeattemptsall'] = 'Todos los ({$a})';
$string['gradeattemptsautograded'] = 'Los que han sido automáticamente calificados ({$a})';
$string['gradeattemptsmanuallygraded'] = 'Los que han sido  calificados manualmente con anterioridad ({$a})';
$string['gradeattemptsneedsgrading'] = 'Los que necesitan calificarse  ({$a})';
$string['graded'] = '(calificado)';
$string['gradenextungraded'] = 'Calificar los siguientes {$a} intentos no calificados';
$string['gradeungraded'] = 'Calificar todos los  {$a} intentos no-calificados';
$string['grading'] = 'Calificación manual';
$string['gradingall'] = 'Todos los {$a}  intentos de contestar esta pregunta.';
$string['gradingattempt'] = 'Intento número {$a->attempt} de {$a->fullname}.';
$string['gradingattemptsxtoyofz'] = 'Intentos de calificación {$a->from} a  {$a->to} de {$a->of}';
$string['gradingattemptwithidnumber'] = 'Intento número {$a->attempt} para {$a->fullname} ({$a->idnumber})';
$string['grading:componentname'] = 'Reporte de calificación manual';
$string['gradingnextungraded'] = 'Los siguientes {$a} intentos no calificados';
$string['gradingnotallowed'] = 'No tiene permiso para calificar manualmente las respuestas de este examen';
$string['gradingquestionx'] = 'Calificando las preguntas {$a->number}: {$a->questionname}';
$string['gradingreport'] = 'Reporte de calificación manual';
$string['gradingungraded'] = '{$a} intentos no calificados';
$string['gradinguser'] = 'Intentos para {$a}';
$string['grading:viewidnumber'] = 'Ver los numerosID de los estudiantes mientras califica';
$string['grading:viewstudentnames'] = 'Ver los nombres de los estudiantes mientras califica';
$string['hideautomaticallygraded'] = 'Ocultar preguntas que fueron calificadas de forma automática';
$string['inprogress'] = 'En curso';
$string['invalidattemptid'] = 'No existe la ID de tal intento';
$string['invalidquestionid'] = 'No se ha encontrado una pregunta calificable con la ID {$a}';
$string['noquestionsfound'] = 'No se encontraron preguntas para calificar manualmente';
$string['nothingfound'] = 'Nada para mostrar';
$string['options'] = 'Opciones';
$string['orderattempts'] = 'Ordenar los intentos';
$string['pluginname'] = 'Calificación manual';
$string['privacy:metadata'] = 'El plugin de Calificación Manual del examen no almacena ningún dato personal. Proporciona una interfaz para que los usuarios almacenen datos sin almacenar ningunos datos por sí mismo.';
$string['qno'] = 'Q #';
$string['questionname'] = 'Nombre de la pregunta';
$string['questionsperpage'] = 'Preguntas por página';
$string['questionsthatneedgrading'] = 'Preguntas que requieren calificación';
$string['questiontitle'] = 'Pregunta {$a->number} : "{$a->name}" ({$a->openspan}{$a->gradedattempts}{$a->closespan} / {$a->totalattempts} intentos {$a->openspan}graded{$a->closespan}).';
$string['randomly'] = 'Aleatoriamente';
$string['saveandnext'] = 'Guardar y pasar a la siguiente página';
$string['showstudentnames'] = 'Mostrar los nombres de los estudiantes';
$string['tograde'] = 'Para calificar';
$string['total'] = 'Total';
$string['unknownquestion'] = 'Preguntas desconocidas';
$string['updategrade'] = 'actualizar calificaciones';
