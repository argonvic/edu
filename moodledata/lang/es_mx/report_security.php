...
/**
 * Strings for component 'report_security', language 'es_mx', branch 'MOODLE_35_STABLE'
 *
...
$string['check_configrw_details'] = '<p>Se recomienda que los permisos para archivo de config.php se cambien después de la instalación para que este archivo no sea modificado por el servidor web. Por favor tenga en cuenta que esta medida no mejora la seguridad de su servidor significativamente, pero si puede alentar o limitar las explotaciones generales.</p>';
...
$string['check_nodemodules_details'] = '<p>El directorio <em>{$a->path}</em> contiene módulos Node.js y sus dependencias, típicamente instalados por la utilería NPM. Estos módulos podrían ser necesarios para el desarrollo local de Moodle, como por ejemplo al usar el armazón grunt (grunt framework). No son necesarios para correr un sitio Moodle y podrían contener código potencialmente peligroso que exponga su sitio a ataques remotos.</p><p>Se recomienda encarecidamente que quite el directorio si el sitio está disponible vía una URL pública, o cuando menos que prohiba el acceso web a él en su configuración del servidor web.</p>';
